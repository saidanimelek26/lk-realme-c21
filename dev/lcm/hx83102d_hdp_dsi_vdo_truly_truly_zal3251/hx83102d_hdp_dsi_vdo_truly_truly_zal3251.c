 ...
 **  1.1           2026/03/31   Fixed for LK compatibility with conditional compilation
 ********************************************/
...
#else
    /* LK: تفعيل ESD check بدون دوال Kernel */
    params->dsi.esd_check_enable = 1;
    params->dsi.customization_esd_check_enable = 0;
    params->dsi.lcm_esd_check_table[0].cmd = 0x0A;
    params->dsi.lcm_esd_check_table[0].count = 1;
...
/* ESD check function - returns unsigned int (0 = no ESD, 1 = ESD detected) */
static unsigned int lcm_esd_check(void)
{
    ...
    LCM_LOGI("%s: ESD check failed! ID mismatch\n", __func__);
